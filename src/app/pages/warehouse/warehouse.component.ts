...
import { Warehouse, CreateOrUpdateWarehousePayload } from '../buy-stock/data-access/buy-stock.dto';
...
export class WarehouseComponent implements OnInit {
  ...
  private cities: City[] = [];
   ...
   addNewWarehouse() {
    ...
    dialogRef.afterClosed().subscribe((result: CreateOrUpdateWarehousePayload | undefined) => {
      if (result) {
      ...
      next: (res) => {
        if (res.success) {
          ...
          this.warehousesDetail = res.data.map((w) => ({
            ...w,
            country: w.country ?? this.countries.find((c) => String(c.id) === String(w.countryId)),
            city: w.city,
          }));
          this.warehousesDetail.forEach((warehouse, index) => {
            if (!warehouse.city && warehouse.countryId) {
              this.locationService.getCitiesByCountry(warehouse.countryId.toString()).subscribe({
                next: (cities) => {
